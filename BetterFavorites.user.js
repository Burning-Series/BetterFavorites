// ==UserScript==
// @name         BS Favoriten Funktion
// @namespace    http://tampermonkey.net/
// @version      0.4
// @description  Favoritenbearbeitung direkt bei der jeweiligen Serie im Header!
...
(function addseries() {
	...
	document.getElementsByTagName("nav")[0].children[0].insertBefore(btn,document.getElementsByTagName("nav")[0].children[0].children[2]);// bar at the top
	if(animedifference){
...
function changeList(btn,btnB,ar,isRemoving,toAnime){
	btn.textContent=isRemoving?"Hinzufügen":"Entfernen";
	if(animedifference){
	...
	if((getIndex(ar,b,"",true)>-1)==isRemoving){
		if(!isRemoving){
			if(toAnime||!animedifference){
				...
				//ar=scripts.AT.insertBefore(ar,b,"AnimeBorder");
			}
		}else{
			ar=removeArray(ar,b);
		}
